...
const kinds = new Set(['concept', 'journal', 'weekly', 'project']);
...
function parseNote(id, text) {
  ...
  function visit(node) {
    if (node.type === 'heading') {
      ...
      if (collection) collection.push({ id: `${id}#${anchor}`, title: title.slice(4),
        url: `${url}#${anchor}`, topic: meta.tags?.[0] ?? '', sourceTitle: meta.title });
    }
    if (node.type === 'text') {
  ...
  return { note: { id, title: meta.title, description: (meta.description ?? textOf(firstParagraph ?? {}).slice(0, 150)).trim(),
    date: meta.date, published: meta.published ?? meta.date, type: meta.type, topics: meta.tags ?? [],
...
if (process.argv.includes('--check')) {
  ...
  const data = { notes: pages.map(p => p.note), questions: pages.flatMap(p => p.questions), quizzes: pages.flatMap(p => p.quizzes) };
