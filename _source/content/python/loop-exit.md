...
찾는 값을 발견하면 탐색을 멈추고, 끝까지 못 찾았을 때만 실패를 알리고 싶다면 `for`와 `else`를 함께 쓸 수 있다. [[learning/2026-09-18-loop-exit|프로그램을 종료해야 하는지 물었던 학습 기록]]에서 이어진 개념이다.
...
## 퀴즈: break 뒤의 코드는 실행될까?

위 예시에서 찾는 값을 `4`에서 `9`로 바꾸면 무엇이 어떤 순서로 출력될까? 답을 열기 전에 `else`에 도달하는 이유를 설명해 보자.
